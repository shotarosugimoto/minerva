...
def select_tool(openai_api_key: str, goal: str, now_task_element: TaskTreeElement, needed_information: str, question: list[str]):
    openai.api_key = openai_api_key

    questions = ''
    for i in range(len(question)):
        questions += f'{i + 1}: {question[i]}'
    ...
    determine whether [questions] should be asked to the user or GPT-3.5.
    If [questions] should be asked to the user, output only U.
    If [questions] should be asked to GPT-3.5", output only G
    ...
    '''
        ...
        print(f"（確認用）tools(row): {ai_response}")
        # トークン数のアウトプットの処理
